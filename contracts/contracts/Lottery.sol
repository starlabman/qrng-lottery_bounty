...
contract Lottery is RrpRequesterV0, Ownable {
  ...
  uint256 public ticketPrice = 0.001 ether; // price of a single ticket
  uint256 public week = 1; // current week counter
  uint256 public endTime; // datetime that current week ends and lottery is closable
  uint256 public constant MAX_NUMBER = 50; // highest possible number
  ...
  mapping(uint256 => uint256) public winningPot; // mapping to store each weeks pot earnings (if any)
  ...
  constructor(address _airnodeRrpAddress) RrpRequesterV0(_airnodeRrpAddress) {
    endTime = block.timestamp + 3 minutes; // store the end time of the lottery
  }
  ...
  /// @param _numbers The participant's chosen lottery number for which they're buying a ticket
  function enter(uint256[] calldata _numbers, uint256 numberOfTickets) external payable {
    if (block.timestamp >= endTime) revert EndTimeReached(endTime); // lottery has to be open
    require(numberOfTickets < 10, "can buy a maximum of 10 tickets");
    require(_numbers.length == 5, "can submit a maximum of 5 numbers in every ticket");
    require(msg.value == ticketPrice * numberOfTickets, "Ticket price is 0.001 ether for each ticket"); // user needs to send 0.0001 ether with the transaction
    for (uint i = 0; i < numberOfTickets; i++) {
      for (uint j = 0; j < 5; j++) {
        tickets[week][_numbers[j]].push(msg.sender);
      }
    } // add user's address to list of entries for their number under the current week
    pot += ticketPrice * numberOfTickets; // account for the ticket sale in the pot
  }
  ...
  function getLastNWeeksWinningNumber(uint256 n) public view returns (uint256[] memory winningNumbers) {
    winningNumbers = new uint256[](n);
    for (uint i = 0; i < n; i++) {
      winningNumbers[i] = winningNumber[week - i];
    }
  }

  function getLastNWeeksWinningPot(uint256 n) public view returns (uint256[] memory winningPots) {
    winningPots = new uint256[](n);
    for (uint i = 0; i < n; i++) {
      winningPots[i] = winningPot[week - i];
    }
  }
